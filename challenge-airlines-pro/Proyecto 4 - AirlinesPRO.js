...
const askForNameAndGreet = () => {
  ...
  window.alert(
    `Hello ${userName}! Let me help you with your next trip.\nLet's check the available flights:`
  );
};
...
const showFlights = () => {
  ...
  for (let i = 0; i < flights.length; i++) {
    if (flights[i].scale === true) {
      ...
      console.log(
        `The flight ${flights[i].id} with origin: ${
          flights[i].from
        } and destination: ${flights[i].to} has a cost of ${flights[
          i
...
const lastFlights = () => {
  let lastQuestion = confirm(
    `Dear ${userName}, would you like to view the last 5 flights of the day?`
  );
  ...
  if (lastQuestion) {
    ...
    for (let i = 0; i < lastFlights.length; i++) {
      console.log(
        `Departure from ${lastFlights[i].from} to ${lastFlights[i].to}.`
      );
    }
  }
};
...
const askForActionAdmin = () => {
  const action = prompt(
    "Do you want to CREATE a new flight or DELETE? [CREATE/DELETE]"
  );
...
const adminRole = () => {
  ...
  if (adminRoleResult === "CREATE") {
    ...
    newFlight.scale = prompt(
      "Are there any stopovers on this flight, true or false? [true/false]"
    );
    ...
    const flightsToDelete = +prompt(
      "Enter the ID of the flight you wish to delete:"
    );
    flights.splice(flightsToDelete, 1);
    window.alert(
      `The flight with ID ${flightsToDelete} has been successfully deleted.`
    );
    console.log(
      `✈ Update: Flight with ID ${flightsToDelete} has been deleted.`
    );
    showFlights();
    ...
    if (flight.cost <= askForPrice) {
      ...
      console.log(
        `The flight ${flight.id} with origin: ${flight.from} and destination: ${
          flight.to
        } has a cost of ${flight.cost.toLocaleString("es-ES", {
          style: "currency",
          currency: "EUR",
        })}.`
      );
...
const playAirline = () => {
  ...
  showFlights();
  const average =
    flights.reduce((total, next) => total + next.cost, 0) / flights.length; //to calculate the average
  console.log(
    `✈ The average price of flights is ${average.toLocaleString("es-ES", {
      style: "currency",
      currency: "EUR",
    })} and there are ${withScale.length} flights with stopovers.`
  );
  lastFlights();
