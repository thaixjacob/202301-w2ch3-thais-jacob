...
const askForUserNameAndGreet =() => { //bienvenida
...
const randomNumbersCard = () => { //crea numeros randoms 
    let randomNumber = Math.ceil(Math.random() * 99) + 1
    return randomNumber
};

const createCard = () => { //crea el carton
    while (bingoCard.length < 15) {
        const bingoCardNumbers = {
            number: randomNumbersCard(), matched: false,
        }
...
const showAndConfirmCard = () => { //enseña y pregunta al usuario si desea el carton
    console.log("Your numbers are:")
    console.table(bingoCard)
    console.log("**************")
    
    let confirmCard = confirm("Do you want change your card?\n\nACCEPT, if yes\nCANCEL, to start the game\n")
    if (confirmCard === true){
        deleteCard()
        createCard()
    }
...
const callNumber = () => { //llama a un numero aleatorio
...
const checkNumberCard = () => { //verifica si el numero coincide con el carton
    const bingoNumber = callNumber()
    for (let i = 0; i < bingoCard.length; i++) {
        var index = bingoCard.indexOfNumber(bingoNumber);
        if (index !== -1){
            bingoCard[index].matched = true;
            bingoCard[index].number = "X";
            alert(`The number is: ${bingoNumber}...\nWait... WOW! You have this number!`)
...
const showCard = () => { //enseña el carton
    console.table(bingoCard)
}
...
const deleteCard = () => { //borrar carton
